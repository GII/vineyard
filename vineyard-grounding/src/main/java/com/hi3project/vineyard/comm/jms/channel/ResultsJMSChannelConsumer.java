 ...
 * @version 1
 */
public class ResultsJMSChannelConsumer extends AbstractJMSChannelConsumer
{

    public ResultsJMSChannelConsumer(
    ...
    protected MessageConsumer initializeMessageConsumer(String name) throws ServiceGroundingException
    {
        try
        {
            ...
            return this.messageConsumer = this.session.createConsumer(queue);
        } catch (JMSException ex)
        {
            throw new ServiceGroundingException("Cannot create queue for: " + name, ex);
        }
    }
